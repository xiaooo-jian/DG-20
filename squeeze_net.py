...
class SqueezeNet(nn.Module):
    ...
    def __init__(self,
        ...
        if version == 1.1:
            self.features = nn.Sequential(
                nn.Conv3d(4, 64, kernel_size=3, stride=(1, 2, 2), padding=(1, 1, 1)),
                nn.BatchNorm3d(64),
                nn.ReLU(inplace=True),
                nn.MaxPool3d(kernel_size=3, stride=2, padding=1),
                Fire(64, 16, 64, 64),
        ...
        self.result = nn.Linear(130, num_classes)
...
if __name__ == '__main__':
    model = SqueezeNet(version=1.1, sample_size=256, sample_duration=50, num_classes=13)
    model = model.cuda()
    model = nn.DataParallel(model, device_ids=None)
    # print(model)

    input_var = Variable(torch.randn(1, 4, 50, 256, 256))
